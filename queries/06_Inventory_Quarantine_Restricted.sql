...
SELECT 
    i.ITEMNMBR,
    i.LOCNCODE AS Location,
    (i.QTYRECVD - i.QTYSOLD - i.ATYALLOC) AS Available_Qty,  -- Available = received - sold - allocated
    CASE 
        WHEN i.QTYRECVD > i.QTYSOLD THEN 'WFQ'   -- Wait for Quality: receipts > sales
        ELSE 'RMQTY'                              -- Restricted Material Quality
    END AS Restriction_Type,
    -- Hold periods: WFQ = 14 days, RMQTY = 7 days
    DATEADD(DAY, 
        CASE WHEN i.QTYRECVD > i.QTYSOLD THEN 14 ELSE 7 END,
        GETDATE()
    ) AS Hold_Until,
    'Quality Review' AS Reason_Code,
    i.RCTSEQNM AS Receipt_Reference,
    GETDATE() AS Assessed_Date
FROM dbo.IV00300 i
WHERE (i.QTYRECVD - i.QTYSOLD - i.ATYALLOC) > 0  -- Only positive available quantities
